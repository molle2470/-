...
def test_unschedule_product_job_not_found_no_exception():
  """없는 job 제거 시 JobLookupError를 잡아 예외 없이 정상 반환"""
  scheduler = MonitoringScheduler(MagicMock(), MagicMock(), MagicMock())

  with patch.object(scheduler.scheduler, "remove_job", side_effect=JobLookupError("product_99")):
    # 예외가 발생하지 않아야 함
    scheduler.unschedule_product(product_id=99)
...
async def test_monitor_product_price_change():
  """가격 변동 감지 시 MarketSyncService.sync_price_change 호출 및 DB 업데이트"""
  ...
  with (
    ...
    # 가격 변동 후 DB 업데이트 확인
    assert mock_product.original_price == 90000
    mock_session.commit.assert_called()


@pytest.mark.asyncio
async def test_monitor_product_stock_change():
  """재고 변동 감지 시 MarketSyncService.sync_stock_change 호출 및 DB 업데이트"""
  mock_session = AsyncMock()
  mock_session_factory = MagicMock()
  mock_ctx = AsyncMock()
  mock_ctx.__aenter__ = AsyncMock(return_value=mock_session)
  ...
  with (
    ...
    # 재고 변동 후 DB 업데이트 확인
    assert mock_product.stock_status == StockStatusEnum.OUT_OF_STOCK
    mock_session.commit.assert_called()
